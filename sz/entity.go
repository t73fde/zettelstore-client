...
func ScanEntity(inp *input.Input) (res string, success bool) {
	...
	if inp.Ch == '#' {
		...
		if inp.Ch == 'x' || inp.Ch == 'X' {
			return scanEntityBase16(inp)
		}
		return scanEntityBase10(inp)
	}
	return scanEntityNamed(inp, pos)
}

func scanEntityBase16(inp *input.Input) (string, bool) {
	inp.Next()
	if inp.Ch == ';' {
		return "", false
	}
...
func scanEntityBase10(inp *input.Input) (string, bool) {
	// Base 10 code
	if inp.Ch == ';' {
		return "", false
	}
...
func scanEntityNamed(inp *input.Input, pos int) (string, bool) {
	for {
		switch inp.Ch {
		case input.EOS, '\n', '\r', '&':
			return "", false
